...
import {
  ...
  mapActionTypeToActionName,
  TCrudActionTypes,
  mapActionToHTTPMethod,
  availableActions
} from './constants'
...
interface ICreateAPIActionOptions {
  ...
  // The function that triggers actions for the model's relations. Pluggable: defaults to noop.
  getActionsForRelations: IGetActionsForRelations
}
...
// We don't handle relations by default.
const defaultGetActionsForRelations = () => []
...
function createAPIAction({
  ...
  return (payload?: IAPIActionParams) => async (dispatch: Dispatch<any>, getState: () => any) => {
    ...
    const relationKeys: IRelationKeys = {}
    const crudAction = mapActionToActionType[actionName]
    if (payload) {
    ...
    const startActionName = mapActionTypeToActionName.start[crudAction]
    if (resource && startActionName) {
      if (relations && (actionName === 'update' || actionName === 'create')) {
        ...
        for (const i in relations.map) {
          ...
          Object.keys(relationData).forEach(id => {
            ...
            actions.push(
              relations.map[i][mapActionTypeToActionName.start[crudAction]](relationData[id])
            )
          })
        }
        actions.forEach(dispatch)
      } else {
        dispatch(actionCreators[startActionName](localResource))
      }
    }
    ...
    try {
      ...
      const errorActionName = mapActionTypeToActionName.error[crudAction]
      if (method === 'get') {
        dispatch(actionCreators[errorActionName](e.message))
      } else {
        // Methods that persist data require the resource to revert optimistic updates
        dispatch(actionCreators[errorActionName](e.message, localResource))
      }
      throw e
    }
  }
...
export interface IRelationMap {
  schema: Schema
  map: { [relationName: string]: TActionCreators }
}
...
interface IGetActionsForRelations {
  (actionOptions: IActionOptions): Action<unknown>[]
}

const getActionsForIncomingData = (getActionsForRelations: IGetActionsForRelations) => (
  options: IActionOptions
): Action<unknown>[] => {
  const { crudAction, relations, actionName, actionCreators, data, cid } = options
  ...
  if (!relations || (crudAction !== 'fetch' && crudAction !== 'update')) {
    return actionName === 'create'
      ? [actionCreators[successActionName](data, cid)]
      : [actionCreators[successActionName](data)]
  }
  return getActionsForRelations(options)
}
...
type TActionCreators = ReturnType<typeof createActionCreators>
...
function createAPIResource<IResource extends IBaseResource>({
  ...
  selectAuthToken,
  relations,
  getActionsForRelations = defaultGetActionsForRelations,
  options = {
