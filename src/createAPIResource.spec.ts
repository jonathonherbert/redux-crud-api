...
modelResourceWithTransforms = createAPIResource({resourceName, baseUrl, options: {transformIn, transformOut}})
...
describe('(Util) asyncActionCreator', () => {
	...
	it('creates a create watcher saga', () => {
		...
		const expectedYield = call(takeLatest, modelResource.actionNames.create, modelResource.workers.create)
		expect(iterator.next().value).toEqual(expectedYield)
	})

	it('creates a fetch watcher saga', () => {
		const iterator = modelResource.sagas.fetch()
		const expectedYield = call(takeLatest, modelResource.actionNames.fetch, modelResource.workers.fetch)
		expect(iterator.next().value).toEqual(expectedYield)
	})

	it('creates an update watcher saga', () => {
		const iterator = modelResource.sagas.update()
		const expectedYield = call(takeLatest, modelResource.actionNames.update, modelResource.workers.update)
		expect(iterator.next().value).toEqual(expectedYield)
	})
	...
	it('creates a search watcher saga', () => {
		...
		const expectedYield = call(takeLatest, modelResource.actionNames.search, modelResource.workers.search)
		expect(iterator.next().value).toEqual(expectedYield)
	})
	...
	describe('Fetch worker', () => {
		it('makes fetch requests and handles valid array responses', () => {
			...
			expect(iterator.next().value).toEqual(call(fetch, '/api/model/1', {method: 'GET', headers: new Headers()}))
		...
		it('makes fetch requests and applies transforms', () => {
			...
			expect(iterator.next(arrayResponse.json()).value)
				.toEqual(put(actionCreators.fetchSuccess(arrayResponse.json().data.map(data => transformIn(data)))))

			// Resolve the caller Promise via the action meta, which should also return transformed data
			expect(iterator.next().value).toEqual(call(noop, arrayResponse.json().data.map(data => transformIn(data))))
		})
		...
		it('makes fetch requests and handles responses without an envelope', () => {
			...
			expect(iterator.next().value).toEqual(put(actionCreators.fetchStart(resource)))
			expect(iterator.next().value).toEqual(call(fetch, '/api/model/1', {method: 'GET', headers: new Headers()}))
			expect(iterator.next(responseNoEnvelope).value).toEqual(apply(responseNoEnvelope, responseNoEnvelope.json))
			expect(iterator.next(responseNoEnvelope.json()).value)
				.toEqual(put(actionCreators.fetchSuccess(responseNoEnvelope.json())))
		...
		it('makes fetch requests and makes appropriate calls if relations are defined', () => {
			...
			expect(iterator.next().value)
				.toEqual(put(batchActions([actionCreators.fetchSuccess(normalisedModelData.entities.model[1])])))
		...
		it('makes fetch requests to arbitrary endpoints', () => {
			...
			// Use fetch to make the API call
			expect(iterator.next().value)
				.toEqual(call(fetch, '/api/model/recent', {method: 'GET', headers: new Headers()}))
	...
	describe('Update worker', () => {
		it('makes update requests and handles valid responses', () => {
			...
			expect(iterator.next().value).toEqual(call(fetch, '/api/model/1', {
				method: 'PUT',
		...
		it('makes update requests and merges existing model with updates', () => {
			...
			const iterator = modelResource.workers.update(modelResource.actions.update({resource}))
			expect(iterator.next().value).toEqual(select(modelResource.selectors.findById, resource.id))
			expect(iterator.next(resourceFromState).value).toEqual(put(actionCreators.updateStart(resourceFromState)))
			const headers = new Headers()
			headers.append('content-type', 'application/json')
			expect(iterator.next().value).toEqual(call(fetch, '/api/model/1', {
				method: 'PUT',
		...
		it('should throw if the model being updated cannot be found in the local state	', () => {
			const iterator = modelResource.workers.update(modelResource.actions.update({resource}))
			expect(iterator.next().value).toEqual(select(modelResource.selectors.findById, resource.id))
			expect(iterator.next(null).value).toEqual(call(noop, `Could not select model with id ${resource.id}`))
		})
		...
		it('makes update requests and apply transformations', () => {
			...
			expect(iterator.next().value).toEqual(select(modelResourceWithTransforms.selectors.findById, resource.id))
			expect(iterator.next(resource).value).toEqual(put(actionCreators.updateStart(resource)))
			const headers = new Headers()
			headers.append('content-type', 'application/json')
		...
		it('makes update requests and applies relations on optimistic update', () => {
			...
			expect(iterator.next().value).toEqual(select(relationResource.selectors.findById, resource.id))
			expect(iterator.next(resource).value).toEqual(put(batchActions([
			...
			expect(iterator.next(response).value).toEqual(apply(response, response.json))
			expect(iterator.next(response.json()).value).toEqual(put(batchActions([
			...
			expect(iterator.next().value).toEqual(put(batchActions([
	...
	describe('Create worker', () => {
		it('makes create requests and handles valid responses', () => {
			const iterator = modelResource.workers.create(modelResource.actions.create({resource: {...resource, id: 'cid'}}))
			expect(iterator.next().value).toEqual(put(actionCreators.createStart({
				...resource,
				id: 'cid'
			})))
			const headers = new Headers()
			...
			expect(iterator.next().value).toEqual(call(fetch, '/api/model', {
				method: 'POST',
				body: JSON.stringify(bodyContent),
				headers
			}))
			expect(iterator.next(response).value).toEqual(apply(response, response.json))
			expect(iterator.next(response.json()).value).toEqual(put(actionCreators.createSuccess(resource, 'cid')))
			expect(iterator.next().value).toEqual(call(noop, response.json().data))
		})
	...
	describe('Delete worker', () => {
		it('Creates delete requests and handles valid responses', () => {
			...
			expect(iterator.next().value)
				.toEqual(call(fetch, '/api/model/1', {method: 'DELETE', headers: new Headers()}))
			expect(iterator.next({status: 200}).value).toEqual(put(actionCreators.deleteSuccess(resource)))
	...
	describe('Search worker', () => {
		it('Creates search requests and handles valid responses', () => {
			...
			expect(iterator.next().value).toEqual(call(fetch, '/api/model/search?' + qs.stringify(searchParams), {
				method: 'GET',
				headers: new Headers()
			}))
			expect(iterator.next(searchResponse).value).toEqual(apply(searchResponse, searchResponse.json))
			expect(iterator.next(searchResponse.json()).value)
				.toEqual(put(actionCreators.fetchSuccess(searchResponse.json().data)))
